...
const ProfileCard = ({
  ...
  const handleGetToken = () => {
    if (!user.verified) {
      getVerificationToken(user.email)
        .then((res) => {
          setFeedback(res);
        })
      ...
      <div className={classes.profileCard}>
        ...
        <div className={classes["account-info"]}>
          ...
          <Link onClick={handleGetToken}>
            <PiWarningOctagon
              size={20}
              style={{ color: user.verified ? "green" : "#FFD233" }}
            />
            <h6 className={classes.text}>
              {user.verified ? "Verified" : "Account not verified, verify now"}
            </h6>
          </Link>
...
export default connect(mapStateToProps, { handleLogout, getVerificationToken })(
  ProfileCard
);
